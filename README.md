<h1 align="center">Hi, I'm Vishal Joseph Dsouza</h1>
<h3 align="center">Automation &amp; AI Builder · Customer-Focused · ex-Monzo/TSYS fintech · MSc FinTech (Distinction)</h3>

<p align="center">
I build automation and AI tools — and I've spent close to two years in front-line customer support for regulated UK fintech.<br/>
Open to remote roles in automation, AI, customer success/support, and fintech operations.
</p>

---

## About

I design event-driven automations, email-sequencing pipelines, and LLM-powered tooling — and I build and run a live SaaS end-to-end. Before building full-time, I supported customers in regulated UK banking (Monzo, TSYS): a 95% satisfaction score, fraud &amp; chargeback investigation, and work under FCA Consumer Duty. So I pair hands-on building with a real understanding of customers, payments, and compliance.

- 🔭 Building **Veltrix Labs**, a live automation platform → **[veltrixlabs.tech](https://veltrixlabs.tech)**
- 🧰 Stack: Python · n8n · LLM APIs (Claude / OpenAI) · FastAPI · Docker · PostgreSQL / Redis · Stripe · TypeScript / Next.js
- 🌱 Learning: LangChain · LangGraph · RAG · vector databases
- 🎓 MSc Financial Technology (Distinction) · B.Com Accounting &amp; Finance · ACCA Diploma
- 📫 vish12jos2000@gmail.com · [LinkedIn](https://linkedin.com/in/viseph)

---

## Featured projects

- **[customer-lifecycle-engine](https://github.com/itsviseph/customer-lifecycle-engine)** — an explainable **customer-lifecycle automation engine**: scores customer health &amp; churn risk, classifies the lifecycle stage, and fires the right action (onboarding, re-engagement, **CSM hand-off for at-risk accounts**, win-back), with an optional **LLM personalisation** layer. Pure Python, **fully unit-tested**. *(customer success / lifecycle + AI automation)*
- **[algo-trading-system](https://github.com/itsviseph/algo-trading-system)** — ML crypto-futures trading system with an 8-year **walk-forward validation** harness that **caught its own overfitting** and rejected the strategy from live trading — the discipline that separates deployable ML from a backtest illusion. *(Python · scikit-learn · ccxt · Docker)*
- **[coffee-sensory-analysis](https://github.com/itsviseph/coffee-sensory-analysis)** — a coffee cupping/sensory web app (CSV/Excel import, interactive charts, one-click PDF reports) built for a specialty-coffee estate in India. *(React · Chart.js · jsPDF)*
- **Veltrix Labs** — *live; code private* — missed-call → instant-SMS customer recovery for SMEs: 14 event-driven n8n workflows, Twilio/Vonage SMS (A2P 10DLC-compliant), Stripe billing, on a Dockerized Linux VPS → **[veltrixlabs.tech](https://veltrixlabs.tech)**

### Client work (under NDA — code private, available to discuss)
- **LLM document-processing API** for a UK accounting firm — async FastAPI + Tesseract OCR + multi-provider LLM extraction (Claude/OpenAI) with confidence scoring. *(I built the backend &amp; infrastructure.)*
- **Accounts-receivable automation platform** for a UK construction client — Next.js · Supabase · n8n · Xero · Stripe · OpenAI.

---

## Tech

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=for-the-badge&logo=n8n&logoColor=white)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white)

**LLMs &amp; automation:** Anthropic Claude · OpenAI · prompt engineering · multi-provider LLM integration · OCR · n8n · Twilio / Vonage · Stripe · Google APIs

**Customer &amp; domain:** Customer support (email / chat / phone) · onboarding &amp; help content · FCA Consumer Duty · fraud &amp; chargeback investigation · financial-crime flagging &amp; referral · credit-risk &amp; collections · GDPR
